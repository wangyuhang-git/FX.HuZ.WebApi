    ...
    public class DustNewReceiveController : ApiController
    {
        ...
        public IHttpActionResult Post([FromBody] JObject jobject)
        {
            ...
            try
            {
                ...
                //stopwatch.Stop();
                //Helper.SaveLog("新dmp数据保存时长:" + stopwatch.ElapsedMilliseconds + "毫秒", "DustNewReceive");
            }
            catch (Exception ex)
            {
        ...
        private void SetData(DustInfoNewModel dustInfo)
        {
            try
            {
                ...
                {
                    if (string.IsNullOrEmpty(dustInfo.DeviceId))
                    {
                        return;//设备编号为空直接返回
                    }
                    ...
                    if (null == hasData || Convert.IsDBNull(hasData))
                    {
                        ...
                        if (DateTime.Now.AddMinutes(-30) > DateTime.Parse(dustInfo.ReportDate))
                        {
                            ...
                            else
                            {
                                ...
                                if (!returnValue)
                                {
                                    Helper.SaveLog(dustInfo.DeviceId + "设备更新异常设备数据表失败！更新原因：上传时间和当前时间相差大于30分钟！", "DustReceive");
                                }
                            }
                        }
                        //市本级
                        ...
                        else
                        {
                            ...
                            if (dtSN != null && dtSN.Rows.Count == 0)
                            {
                                ...
                                if (returnValue)
                                {
                                    ...
                                    {
                                        Helper.SaveLog(dustInfo.DeviceId + "保存区县项目数据失败", "DustNewReceive");
                                    }
                                    #endregion
                                }
                                else
                            ...
                            else
                            {
                                ...
                                if (returnValue)
                                {
                                    ...
                                    {
                                        Helper.SaveLog(dustInfo.DeviceId + "更新区县项目数据失败", "DustNewReceive");
                                    }
                                    #endregion
                                }
                                else
            ...
            {
                //Helper.SaveLog("Dmp参数：" + JsonConvert.SerializeObject(dustInfo) + ",保存数据异常！" + ex.Message, "DustNewReceive异常");
